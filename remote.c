...
void append_layer(DataLayer* layer) {
    ...
    if(layers_tail == NULL) {
        if(layers_head == NULL) {
            ...
            FURI_LOG_E(
                TAG,
                "Layers has null tail but non-null head! Please report at https://github.com/aaronjamt/Flipper-SubGHz-Plugin/issues/new\n");
...
bool remote_load_layer(const char* name, void** storage) {
    ...
    furi_string_printf(path, "%s/%s", APP_ASSETS_PATH("plugins"), name);
    ...
    if(plugin_manager_load_single(manager, furi_string_get_cstr(path)) == PluginManagerErrorNone) {
        ...
        if(plugin_manager_get_count(manager) == plugin_count + 1) {
            ...
            DataLayerEntryPoint* ep =
                (DataLayerEntryPoint*)plugin_manager_get_ep(manager, plugin_count);
...
void rx_event_callback(void* ctx) {
    ...
    Buffer payload = {.data = NULL, .size = 0};
    ...
    while(layer) {
        ...
        if(layer->recv_buffer.size == 0) {
            ...
            memmove(
                layer->recv_buffer.data,
                layer->recv_buffer.data + processed,
                layer->recv_buffer.size);
...
static int32_t tx_worker(void* context) {
    ...
    while(1) {
        uint32_t events = furi_thread_flags_wait(
            WorkerEventStop | WorkerEventData | WorkerEventFlush | WorkerEventAgain,
            FuriFlagWaitAny,
            1000);
        ...
        if((events & WorkerEventData) &&
           (furi_mutex_acquire(tx_worker_mutex, 25) == FuriStatusOk)) {
        ...
        while(layer) {
            ...
            // Otherwise, move the unprocessed data to the beginning of the buffer
                ...
                memmove(
                    layer->send_buffer.data,
                    layer->send_buffer.data + processed,
                    layer->send_buffer.size);
                layer->send_buffer.data =
                    realloc(layer->send_buffer.data, layer->send_buffer.size);
            }
